...
files = mechanics.io.exportConstitutiveStudyReport(batch, population, inference, config);
...
files = mechanics.plotting.exportConstitutiveStudyFigures(batch, population, inference, config);
...
batch.modelSummary = table("neo-hookean", 4, 1, 'VariableNames', {'ModelName','SelectionCount','SelectionFraction'});
population.parameterObservationCount = 4;
population.specimenCount = 4;
population.parameterTable = table(["S1";"S2";"S3";"S4"], ["A";"A";"B";"B"], repmat("neo-hookean",4,1), repmat("mu",4,1), [10;11;20;21], nan(4,1), nan(4,1), nan(4,1), 'VariableNames', {'SpecimenId','Group','ModelName','Parameter','Value','BootstrapLower','BootstrapMedian','BootstrapUpper'});
population.overallSummary = table("neo-hookean", "mu", 4, 15.5, std([10 11 20 21]), 15.5, 10, 21, std([10 11 20 21])/15.5, true, 'VariableNames', {'ModelName','Parameter','SpecimenCount','Mean','StandardDeviation','Median','Minimum','Maximum','CoefficientOfVariation','MeetsMinimumCount'});
population.groupSummary = table(["A";"B"], repmat("neo-hookean",2,1), repmat("mu",2,1), [2;2], [10.5;20.5], [0.7071;0.7071], [10.5;20.5], [10;20], [11;21], [0.0673;0.0345], [true;true], 'VariableNames', {'Group','ModelName','Parameter','SpecimenCount','Mean','StandardDeviation','Median','Minimum','Maximum','CoefficientOfVariation','MeetsMinimumCount'});
inference.comparisonCount = 1;
inference.successfulComparisonCount = 1;
inference.significantComparisonCount = 1;
inference.comparisonTable = table("neo-hookean", "mu", "A", "B", 2, 2, 10.5, 20.5, -10, -10, -11, -9, -5, -1, 0.01, 0.01, true, "", "", 'VariableNames', {'ModelName','Parameter','Group1','Group2','Group1Count','Group2Count','Group1Mean','Group2Mean','MeanDifference','MedianDifference','ConfidenceIntervalLower','ConfidenceIntervalUpper','HedgesG','CliffsDelta','PermutationPValue','AdjustedPValue','Significant','ErrorIdentifier','ErrorMessage'});
end
