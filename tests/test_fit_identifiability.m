...
diagnostics = mechanics.fitting.analyzeFitIdentifiability(fitResult, uncertainty, localConfig());
...
diagnostics = mechanics.fitting.analyzeFitIdentifiability(fitResult, uncertainty, config);
...
verifyGreaterThan(testCase, abs(diagnostics.correlationMatrix(1,2)), 0.99);
end
...
verifyError(testCase, @() mechanics.fitting.analyzeFitIdentifiability(fitResult, uncertainty, localConfig()), "mechanics:fitting:InsufficientIdentifiabilitySamples");
end
