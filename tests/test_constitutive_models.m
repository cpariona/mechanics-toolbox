...
verifyEqual(testCase, mechanics.models.listModels(), ["neo-hookean", "mooney-rivlin", "yeoh"]);
end
...
lambda = 2;
mu = 3;
verifyEqual(testCase, mechanics.models.evaluateModel("neo-hookean", lambda, mu, context), mu * (lambda - lambda^(-2)), "AbsTol", 1e-12);
end
...
fromEngineering = mechanics.models.evaluateModel("mooney-rivlin", lambda - 1, parameters, context);
context.inputMeasure = "true-strain";
fromTrue = mechanics.models.evaluateModel("mooney-rivlin", log(lambda), parameters, context);
...
verifySize(testCase, mechanics.models.evaluateModel("yeoh", strain, [1, 0.1, 0.01]), size(strain));
end

function testUnknownModelIsRejected(testCase)
verifyError(testCase, @() mechanics.models.modelRegistry("unknown"), "mechanics:models:UnknownModel");
end

function testInvalidStretchIsRejected(testCase)
context.inputMeasure = "stretch";
verifyError(testCase, @() mechanics.models.evaluateModel("neo-hookean", 0, 1, context), "mechanics:models:InvalidStretch");
end

function testInvalidParameterCountIsRejected(testCase)
verifyError(testCase, @() mechanics.models.evaluateModel("mooney-rivlin", 0.1, 1), "mechanics:models:InvalidParameterCount");
end
