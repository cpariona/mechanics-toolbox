...
assessment = mechanics.fitting.assessFitReliability(fitResult, uncertainty, identifiability, windowStability, residualDiagnostics);
...
assessment = mechanics.fitting.assessFitReliability(fitResult, struct(), identifiability, windowStability, struct());
...
assessment = mechanics.fitting.assessFitReliability(fitResult, struct(), struct(), struct(), struct(), config);
verifyEqual(testCase, assessment.status, "incomplete");
end

function testExportCreatesFiles(testCase)
assessment = mechanics.fitting.assessFitReliability(localFitResult(true, 0.03, 0.99));
