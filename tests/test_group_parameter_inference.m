function tests = test_group_parameter_inference
...
inference = mechanics.workflow.compareSelectedParametersBetweenGroups(population, localConfig());
...
inference = mechanics.workflow.compareSelectedParametersBetweenGroups(population, config);
...
count = numel(value);
population.parameterTable = table("S" + (1:count)', group, repmat("neo-hookean",count,1), repmat("mu",count,1), value, nan(count,1), nan(count,1), nan(count,1), 'VariableNames', {'SpecimenId','Group','ModelName','Parameter','Value','BootstrapLower','BootstrapMedian','BootstrapUpper'});
end
