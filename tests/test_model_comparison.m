function tests = test_model_comparison
...
comparison = mechanics.workflow.compareModelsWithDiagnostics(["neo-hookean", "mooney-rivlin"], strain, stress, context, mechanics.config.fittingConfig(), localFastConfig());
...
comparison = mechanics.workflow.compareModelsWithDiagnostics(["neo-hookean", "not-a-model"], strain, stress, context, mechanics.config.fittingConfig(), config);
...
verifyError(testCase, @() mechanics.workflow.compareModelsWithDiagnostics("neo-hookean", strain, stress, context, mechanics.config.fittingConfig(), config), "mechanics:workflow:UnknownModelSelectionCriterion");
end
...
measuredStress = mechanics.models.evaluateModel("neo-hookean", strain, 15, context);
end
