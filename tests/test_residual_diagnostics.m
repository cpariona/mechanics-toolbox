...
fitResult = localFitResult([0.02; -0.01; 0.03; -0.02; 0.01; -0.03; 0.02; 0.00; -0.01; 0.01]);
...
x = linspace(0, 1, 20)';
residual = 0.2 .* x - 0.1;
diagnostics = mechanics.fitting.analyzeFitResiduals(localFitResult(residual, x));
...
diagnostics = mechanics.fitting.analyzeFitResiduals(localFitResult(residual), config);
...
verifyError(testCase, @() mechanics.fitting.analyzeFitResiduals(localFitResult([0.1; -0.1; 0.05])), "mechanics:fitting:InsufficientResidualObservations");
end

function testExportCreatesFiles(testCase)
diagnostics = mechanics.fitting.analyzeFitResiduals(localFitResult(linspace(-0.05, 0.05, 12)'));
