...
stability = mechanics.fitting.analyzeFitWindowStability("neo-hookean", x, y, context, fitConfig, config);
verifyEqual(testCase, stability.successfulWindowCount, numel(config.windowFractions));
verifySize(testCase, stability.parameterMatrix, [numel(config.windowFractions), 1]);
verifyTrue(testCase, stability.stable);
verifyLessThan(testCase, stability.parameterSummary.RelativeRange, 0.05);
end

function testWindowSummaryIsCreated(testCase)
[x, y, context] = localNeoHookeanData();
stability = mechanics.fitting.analyzeFitWindowStability("neo-hookean", x, y, context, mechanics.config.fittingConfig(), localConfig());
verifyEqual(testCase, height(stability.windowSummary), 4);
verifyEqual(testCase, stability.windowSummary.WindowFraction, localConfig().windowFractions(:));
...
verifyError(testCase, @() mechanics.fitting.analyzeFitWindowStability("neo-hookean", x, y, context, mechanics.config.fittingConfig(), config), "mechanics:fitting:InvalidWindowFractions");
end
...
y = mechanics.models.evaluateModel("neo-hookean", x, 12, context);
end
