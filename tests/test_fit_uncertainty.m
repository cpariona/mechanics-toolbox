function tests = test_fit_uncertainty
...
config = localConfig();
uncertainty = mechanics.fitting.bootstrapFitUncertainty(fitResult, config);
verifyEqual(testCase, uncertainty.successfulCount, config.sampleCount);
verifySize(testCase, uncertainty.parameterSamples, [config.sampleCount, 1]);
verifyLessThan(testCase, uncertainty.parameterLower, uncertainty.parameterUpper);
verifyGreaterThan(testCase, fitResult.parameters, uncertainty.parameterLower);
verifyLessThan(testCase, fitResult.parameters, uncertainty.parameterUpper);
end
...
verifyTrue(testCase, all(uncertainty.predictionUpper >= uncertainty.predictionLower));
end
...
verifyError(testCase, @() mechanics.fitting.bootstrapFitUncertainty(fitResult, config), "mechanics:fitting:UnknownBootstrapMethod");
end

function testUncertaintyExport(testCase)
fitResult = localFitResult();
uncertainty = mechanics.fitting.bootstrapFitUncertainty(fitResult, localConfig());
folder = string(tempname);
cleanup = onCleanup(@() localDeleteFolder(folder)); %#ok<NASGU>
files = mechanics.io.exportFitUncertainty(fitResult, uncertainty, folder);
...
trueStress = mechanics.models.evaluateModel("neo-hookean", strain, 12, context);
...
fitResult = mechanics.fitting.fitModel("neo-hookean", strain, measuredStress, context, fitConfig);
end
