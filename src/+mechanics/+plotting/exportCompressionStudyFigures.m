...
if config.includePopulationCurve && study.populationStatus == "completed"
    ...
    if all(isfinite(curves.confidenceLower)) && all(isfinite(curves.confidenceUpper))
        lower = -curves.confidenceUpper(order);
        upper = -curves.confidenceLower(order);
        fill(ax, [x; flipud(x)], [lower; flipud(upper)], [0.85 0.85 0.85], ...
...
if config.includeCycleDiagnostics && ~isempty(processedIndices)
    ...
    for outputIndex = 1:numel(processedIndices)
        ...
        xlabel(ax, "Compression displacement magnitude"); ylabel(ax, "Compression force magnitude");
...
titleText = localStudyTitle(study, config); specimen = study.specimen; units = specimen.processed.units;
...
unit = string(unit); if strlength(unit) == 0, label = string(name); else, label = string(name) + " [" + unit + "]"; end
end
