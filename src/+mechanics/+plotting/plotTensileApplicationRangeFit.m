...
for index = 1:numel(specimens)
    ...
    plot(fitAxes, specimen.Deformation, specimen.MeasuredStress, "-", ...
        ...
        "DisplayName", specimen.SourceSpecimenId + " measured");
end
...
for index = 1:numel(specimens)
    ...
    plot(residualAxes, specimen.Deformation, residual, "-", ...
        ...
        "DisplayName", specimen.SourceSpecimenId);
end
...
ylabel(residualAxes, mechanics.plotting.mechanicalAxisLabel( ...
    "residual", context.stressMeasure, stressUnit));
title(residualAxes, "Residual = measured - shared prediction");
...
if ~all(isfield(result, required)) || ...
        ~all(isfield(result.selectedFit, ["specimens", "parameters"])) || ...
        isempty(result.selectedFit.specimens)
    error("mechanics:plotting:InvalidTensileApplicationRangeResult", ...
        "Provide a completed tensile application-range result.");
end
requiredSpecimen = ["SourceSpecimenId", "Deformation", ...
    "MeasuredStress", "Context", "StrainUnit", "StressUnit"];
if ~all(isfield(result.selectedFit.specimens, requiredSpecimen))
    error("mechanics:plotting:IncompleteTensileApplicationRangeFit", ...
        "Selected tensile specimens are missing plotting fields.");
end
end
...
for index = 2:numel(specimens)
    ...
    if current.StrainUnit ~= reference.StrainUnit || ...
        ...
        error("mechanics:plotting:InconsistentTensileApplicationRangeMetadata", ...
            "Tensile fit specimens must share units and constitutive measures.");
    end
end
end
