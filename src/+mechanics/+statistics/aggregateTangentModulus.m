...
for index = 1:curveCount
    ...
    if ~isfield(tangent, "strain") || ...
        ...
        error("mechanics:statistics:MissingTangentModulusCurve", ...
            "Every tangent-modulus result must contain strain and tangentModulusForPlot.");
    end
...
if any(specimenCountByPoint < minimumSpecimens)
    error("mechanics:statistics:InsufficientTangentModulusSupport", ...
        "The selected strain interval must be supported by at least %d specimens at every grid point.", ...
        minimumSpecimens);
end
...
switch rangeMode
    ...
    case "common-overlap"
        ...
        if isempty(runs)
            error("mechanics:statistics:InsufficientTangentModulusSupport", ...
                "No strain interval is supported by at least %d specimens.", ...
                minimumSpecimens);
        end
...
if supportAtStart < minimumSpecimens || supportAtEnd < minimumSpecimens
    error("mechanics:statistics:TangentModulusRangeOutsideSupport", ...
        "The requested strain range must be supported by at least %d specimens.", ...
        minimumSpecimens);
end
end
