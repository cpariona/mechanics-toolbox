...
selectedParameters = table( ...
    ...
    repmat(units.stress, parameterCount, 1), ...
    'VariableNames', {'Parameter','Estimate','Unit'});
...
referenceProperties = table( ...
    ...
    repmat(units.stress, propertyCount, 1), ...
    'VariableNames', {'Property','Estimate','Unit'});
...
sensitivitySummary.MaximumDeformationUnit = ...
    repmat(units.strain, height(sensitivitySummary), 1);
sensitivitySummary.Mu0Unit = ...
    repmat(units.stress, height(sensitivitySummary), 1);
...
if isfield(result, "hasCompressionValidation") && result.hasCompressionValidation
    ...
    compressionSummary.StrainUnit = ...
        repmat(compressionUnits.strain, height(compressionSummary), 1);
    compressionSummary.StressUnit = ...
        repmat(compressionUnits.stress, height(compressionSummary), 1);
...
localWriteReport(reportFile, result, selectedParameters, referenceProperties, ...
    specimenSummary, sensitivitySummary, compressionSummary, outputFiles, units);
...
outputFiles.tensileFitFigure = mechanics.plotting.exportFigureFiles( ...
    fitFigure, folder, "tensile_fit_and_residuals", "png", 200);
outputFiles.tensileFitFigureFig = ...
    string(fullfile(folder, "tensile_fit_and_residuals.fig"));
clear cleanup
...
outputFiles.rangeSensitivityFigure = mechanics.plotting.exportFigureFiles( ...
    sensitivityFigure, folder, "range_sensitivity", "png", 200);
outputFiles.rangeSensitivityFigureFig = ...
    string(fullfile(folder, "range_sensitivity.fig"));
clear cleanup
if isfield(result, "hasCompressionValidation") && result.hasCompressionValidation
    compressionFigure = ...
    ...
    outputFiles.compressionValidationFigure = ...
        mechanics.plotting.exportFigureFiles( ...
        compressionFigure, folder, "compression_validation", "png", 200);
    outputFiles.compressionValidationFigureFig = ...
        string(fullfile(folder, "compression_validation.fig"));
    clear cleanup
end
end
...
if isempty(specimens) || ~all(isfield(specimens, ["StrainUnit","StressUnit"]))
    error("mechanics:io:MissingTensileApplicationRangeUnits", ...
        "Selected tensile specimens must retain strain and stress units.");
end
storedStrainUnit = string(specimens(1).StrainUnit);
storedStressUnit = string(specimens(1).StressUnit);
for index = 2:numel(specimens)
    if string(specimens(index).StrainUnit) ~= storedStrainUnit || ...
            string(specimens(index).StressUnit) ~= storedStressUnit
        error("mechanics:io:InconsistentTensileApplicationRangeUnits", ...
            "Selected tensile specimens must share units.");
    end
end
units.strain = mechanics.plotting.mechanicalDisplayUnit( ...
    "deformation", storedStrainUnit);
units.stress = mechanics.plotting.mechanicalDisplayUnit( ...
    "stress", storedStressUnit);
end
...
for index = 2:numel(specimens)
    if string(specimens(index).StrainUnit) ~= storedStrainUnit || ...
            string(specimens(index).StressUnit) ~= storedStressUnit
        error("mechanics:io:InconsistentCompressionValidationUnits", ...
            "Compression validation specimens must share units.");
    end
end
units.strain = mechanics.plotting.mechanicalDisplayUnit( ...
    "deformation", storedStrainUnit);
